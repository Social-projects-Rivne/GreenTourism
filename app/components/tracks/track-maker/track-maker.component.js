    ...
    controller: function trackMakerController($http, $rootScope) {
        ...
        ctrlScope.addPointForNewTrack = function(e) {
            if (newTrackPointsCounter == 0) {
                newTrackFirstPoint = L.marker([e.latlng.lat, e.latlng.lng], {icon: startIcon}).addTo($rootScope.map);
                ...
                newTrack = L.polyline(ctrlScope.newTrackPoints, {color: "#000", opacity: 1}).addTo($rootScope.map);
            }
        }
        ...
        ctrlScope.startStepTwo = function () {
            if (newTrackPointsCounter > 1) {
                ...
                newTrackLastPoint = L.marker([ctrlScope.newTrackPoints[ctrlScope.newTrackPoints.length-1][0], ctrlScope.newTrackPoints[ctrlScope.newTrackPoints.length-1][1]], {icon: finishIcon}).addTo($rootScope.map);
                $rootScope.map.off('click', ctrlScope.addPointForNewTrack);
                ctrlScope.trackPointsAlert = false;
        ...
        ctrlScope.selectNewPlaceType = function () {
            for (var i = 0; i < ctrlScope.types.length; i++) {
                if (ctrlScope.newTrackType.type == ctrlScope.types[i].type) {
                    $rootScope.map.removeLayer(newTrack);  
                    newTrack = L.polyline(ctrlScope.newTrackPoints, {color: ctrlScope.newTrackType.color, opacity: 1}).addTo($rootScope.map);
                }
            }
        }
