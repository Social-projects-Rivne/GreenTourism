...
angular.module('eventsMap', ['calendar','eventMapType'])
    ...
    controller: ['$scope','$routeParams','$http','eventMapService','eventList',
        function ($scope,$routeParams,$http,eventMapService,eventList) {
             ...
             if (!$routeParams.lat) this.lat=50.6234; else $scope.mapDanni.x=$routeParams.lat ;
             this.lng = $routeParams.lng;
             if (!$routeParams.lng) this.lng=26.2189; else $scope.mapDanni.y=$routeParams.lng ;
            ...
            $scope.map = L.map('mapevent', {center: [50.6234, 26.2189],zoom: 14});
            $scope.groups = L.layerGroup();

            var Outdoors = L.tileLayer('http://{s}.tile.thunderforest.com/outdoors/{z}/{x}/{y}.png', {attribution: '&copy; <a href="http://www.thunderforest.com/">Thunderforest</a>, &copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'});
            var Streets = L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {attribution: '&copy; <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'});
            var Satellite = L.tileLayer('http://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {attribution: 'Tiles &copy; Esri &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community'});
            ...
            switch ($scope.mapDanni.layerMap) {
                case 'Satellite' :  $scope.map.addLayer(Satellite);
                case 'Streets' :  $scope.map.addLayer(Streets);
                case 'Outdoors' :  $scope.map.addLayer(Outdoors);
                default : $scope.map.addLayer(Outdoors);
            }
...
angular.module('eventsMap').service('eventMapService', function ($http){
